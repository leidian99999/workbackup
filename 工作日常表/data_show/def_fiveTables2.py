...
# 25日省产品计算
def twoFive(df,name="产品"): # 或者"省份"
    df["发货率"] = df["发货量"] / (df["来单量"]+0.00001)
    df["异常筛查"] = df["来单量"] - df["发货量"]
    df["异常筛查率"] = df["异常筛查"] / (df["发货量"] + 0.00001)
    df["签收率"] = df["签收量"] / (df["发货量"] + 0.00001)
    df["未签收量"] = df["发货量"] - df["签收量"]
    df["未签收率"] = df["未签收量"] / (df["发货量"] + 0.00001)
    df["签收激活率"] = df["总激活量"] / (df["签收量"] + 0.00001)
    df["未激活量"] = df["签收量"] - df["总激活量"]
    df["签收未激活率"] = df["未激活量"] / (df["签收量"] + 0.00001)
    df["发货激活率"] = df["总激活量"] / (df["发货量"] + 0.00001)
    df25 = df[[name,"来单量",
               "发货量","发货率","异常筛查","异常筛查率",
               "签收量","签收率","未签收量","未签收率",
               "总激活量","签收激活率","未激活量","签收未激活率",
               "发货激活率"]]
    return df25
